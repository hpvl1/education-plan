  ...
  <q-page>
    ...
    >
      <template v-slot:top-left>
        <q-btn to="/eduplan/" label="Создать план" outline color="purple" />
      </template>
      ...
      <template v-slot:body-cell-name1="val">
        {{ val.row.name }}
        <a v-bind:href="'/local/educationplan/cabinet/#/editplan/?planId=' + val.key"><q-icon name="edit"  class="pen" right /></a>
...
export default defineComponent({
  ...
  async mounted() {
    let comps = await this.fetchPlans();
  ...
  methods: {
    ...
    remove: async function (val) {
      const apiUrl = 'https://edu.zhcom.ru/api/eduplan/delete/' + val.key;
      let res = {};
      const response = await fetch(apiUrl)
        .then(response => response.json())
        .then(aJson => {
